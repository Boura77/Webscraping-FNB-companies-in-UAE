...
for page in [1,2,3]:
    ...
    for items in data["hits"]:
        ...
        item["sale_price"] = items["sale_price"]

        item["stock"] = items["stock"] if 'stock' in items else 0

        if "plp_specifications" in items and "Item Count" in items["plp_specifications"]:

            item["size"] = items["plp_specifications"]["Item Count"]

        if "estimated_delivery" in items:

            item["estimated_delivery"] = items["estimated_delivery"]
...
print(noon)
file_path = "C://Users//singh.b//Downloads//noon1trial.csv"
